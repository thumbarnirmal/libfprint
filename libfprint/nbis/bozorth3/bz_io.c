...
struct xyt_struct * bz_load( const char * xyt_file )
{
   ...
   if ( xytq_s == XYTQ_NULL )
   {
      fprintf( stderr, "%s: ERROR: malloc() failure while loading minutiae buffer failed: %s\n",
                                                     get_progname(),
                                                     strerror(errno)
                                                     );
      return XYT_NULL;
...
struct xyt_struct * bz_prune(struct xytq_struct *xytq_s, int verbose_load)
{
   ...
   {
      if ( verbose_load )
         fprintf( stderr, "%s: WARNING: bz_prune(): trimming minutiae to the %d of highest quality\n",
